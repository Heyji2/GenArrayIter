...
(** The [Iter] module is a small extention of the
    {{:https://ocaml.org/manual/5.2/api/Bigarray.Genarray.html}[BigArray.Genarray]}
    module to complete missing functionalities : iteration, mapping and folding
    over arrays of arbitrary dimensions (Genarrays). A much richer (and heavier)
    library providing (I assume) the same functionality (and much more) in OCaml
    would be {{:https://github.com/owlbarn/owl/}[Owl]}.
    ...
    All these functions are designed to work with Genarrays of arbitrary
    dimensions, and the index is represented as an array of integers. There are
    potentially an infinite number of ways to iterate over a multi-dimensional
    array. The functions in this module use [incr], [decr], [incr_last] and
...
(** [iter f a] applies the function [f] in turn to each element of the array
    [a]. It is equivalent to [f a.(0); f a.(1); ... ; f a.(length a - 1); ()] *)

val iteri : (int Array.t -> 'a -> unit) -> ('a, 'b, 'c) Genarray.t -> unit
(** Same as [iter] except that the function [f] is applied to the index of the
    array as first argument, and to the element itself as second argument.
    [iteri f a] applies the function [f] in turn to each element of the array
    [a] and the index [i] : [f i a.(i)] for all possibles values of the index i.
...
(** [map f k a] applies the map function [f] in turn to each element of the
    array [a] of type ['a] and outputs another array [b], elements of which have
    type ['b]. *)

val map_inplace : ('a -> 'a) -> ('a, 'b, 'c) Genarray.t -> unit
(** Same as [map] but modifies the array {i inplace} instead of creating a new
    one. *)
...
val mapi :
  ...
  ('a, 'k1, 'layout) Genarray.t ->
  ('b, 'k2, 'layout) Genarray.t
(** Same as [map] except that the function [f] is applied to the index of the
    array as the first argument, and to the element itself as second argument.
    [mapi f k a] applies the function [f] in turn to each element of the array
    [a] and the index [i] : [f i a.(i)] for all possibles values of the index
    [i]. The output is a new array.

    {b NB} : the function [incr] is used to iterate over all elements of the
    array [a] *)

val mapi_inplace : (int Array.t -> 'a -> 'a) -> ('a, 'b, 'c) Genarray.t -> unit
(** A combination of [mapi] and [map_inplace] : [mapi_inplace f a] applies [f]
    in turn to each index [i] (as first argument) and element of the array [a]
    (as second argument).
...
(** [fold_left f init a] computes [f (f (... (f init a.(0)) ... ) a.(n-1))]
...
(** [fold_right f a init] computes [f a.(0) (f a.(1) (... (f a.(n-1) init)))]
    ...
    {b NB} : the function [decr] is used to iterate over all elements of the
    array [a] *)
...
(** Given an index {m i(i_0,i_1, ..., i_{n-1})} of dimension
    ...
    - else {m i_0 = 0} and {m i_1} is incremented by one according the same
      alogrithm as {m i_0}.

    So for an index {m i} of dimensions {m d(2,2,3)} in the space
    {m \{0,1\}\times\{0,1\}\times\{0,1,2\}}, applying [incr] to {m i=(0,0,0)}
    several time will give :
    - {m i=(0,0,0)}
    - {m i=(1,0,0)}
    - {m i=(0,1,0)}
    - {m i=(1,1,0)}
    ...
    @param 1
      [index] : an array of size n corresponding to an index of an n dimensions
      Genarray
    @param 2
      [dims] : an array of size n giving all the dimensions of the first
      argument :
      - [index.(0)] goes from [0] to [dims.(0)]
      - [index.(1)] goes from [0] to [dims.(1)]
    @return
      True if the increment succeded. False otherwise, meaning the index has
      reached the end of the array. *)

val incr_last : int array -> int array -> bool
(** Same as [incr] but instead of incrementing the first dimension ({m i_0}) of
    the index {m i(i_0, ..., i_\{n-1\})}, increments the last dimension first
    ({m i_\{n-1\}}) *)
